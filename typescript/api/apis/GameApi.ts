...
export class GameApiRequestFactory extends BaseAPIRequestFactory {
     ...
     * @param name 
     */
    public async getGameByName(authorization: string, name: string, _options?: Configuration): Promise<RequestContext> {
        let _config = _options || this.configuration;

        // verify required parameter 'authorization' is not null or undefined
        if (authorization === null || authorization === undefined) {
            throw new RequiredError("GameApi", "getGameByName", "authorization");
        }


        // verify required parameter 'name' is not null or undefined
        if (name === null || name === undefined) {
            throw new RequiredError("GameApi", "getGameByName", "name");
        }


        // Path Params
        const localVarPath = '/v1/game/name/{name}'
...
export class GameApiResponseProcessor {
     ...
     * to the expected objects
     *
     * @params response Response returned by the server for a request to getGameByName
     * @throws ApiException if the response code was not in [200, 299]
     */
     public async getGameByName(response: ResponseContext): Promise<Game > {
        const contentType = ObjectSerializer.normalizeMediaType(response.headers["content-type"]);
        if (isCodeInRange("200", response.httpStatusCode)) {
            const body: Game = ObjectSerializer.deserialize(
                ObjectSerializer.parse(await response.body.text(), contentType),
